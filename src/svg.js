...
module.exports = function (RED) {
  ...
  jsBeautifyHtmlPath = jsBeautifyHtmlPath.replace(
    'index.js',
    'lib' + path.sep + 'beautify-html.js'
  )
  ...
  jsBeautifyCssPath = jsBeautifyCssPath.replace(
    'index.js',
    'lib' + path.sep + 'beautify-css.js'
  )
  ...
  function HTML(config) {
    ...
    svgString = svgString.replace(
      /(<text.*>)(.*)(<\/text>)/g,
      function (match, $1, $2, $3, offset, input_string) {
    ...
    svgString = svgString.replace(
      ...
      function (match, $1, $2, $3, offset, input_string) {
        if (!config.directory) {
          console.log(
            'For svg node with id=' +
              config.id +
              ' no image directory has been specified'
          )
        ...
        if (!fs.existsSync(absoluteFilePath)) {
          console.log(
            'The specified local image file (' +
              absoluteFilePath +
              ') does not exist'
          )
          // Leave the local file path untouched, since we cannot load the specified image
        ...
        try {
          ...
          return (
            'xlink:href="data:' + contentType + ';base64,' + base64data + '"'
          )
        } catch (err) {
          console.log(
            'Cannot read the specified local image file (' +
              absoluteFilePath +
              '): ' +
              err
          )
    ...
    const DEFAULT_CSS_SVG_NODE = `div.ui-svg svg{
...
div.ui-svg path {
    ...
    var html =
      ...
      panzoomScripts +
      `<div id='tooltip_` +
      config.nodeIdWithoutDot +
      `' display='none' style='z-index: 9999; position: absolute; display: none; background: cornsilk; border: 1px solid black; border-radius: 5px; padding: 2px;'>
</div>
<div class='svggraphics_` +
      config.nodeIdWithoutDot +
      `' style="width:100%; height:100%;">
   <div class='ui-svg' id='svggraphics_` +
      config.nodeIdWithoutDot +
      `' ng-init='init(` +
      configAsJson +
      `)' style="width:100%; height:100%;">` +
  ...
  function SvgGraphicsNode(config) {
    try {
      ...
      while ((match = regularExpression.exec(config.svgString)) !== null) {
        ...
        node.attributeBasedBindings = node.attributeBasedBindings.concat(
          match[1].split(',')
        )
      }
      ...
      node.attributeBasedBindings = node.attributeBasedBindings.filter(
        function (item, index) {
          return node.attributeBasedBindings.indexOf(item) === index
        }
      )

      node.availableCommands = [
        'get_text',
        'update_text',
        'update_innerhtml',
        'update_style',
      ...
      if (checkConfig(node, config)) {
        ...
        var done = ui.addWidget({
          ...
          beforeEmit: function (msg, value) {
            ...
            if (!msg.payload) {
              node.error(
                "A msg.payload is required (msg._msgid = '" + msg._msgid + "')"
              )
              msg.payload = null
            } else {
              if (msg.topic == 'databind') {
                ...
                if (
                  ...
                  node.attributeBasedBindings.length === 0
                ) {
                  node.error(
                    "No bindings have been specified in the config screen or via data-bind-text or via data-bind-values (msg._msgid = '" +
                      msg._msgid +
                      "')"
                  )
                  msg.payload = null
                  ...
                  node.bindings.forEach(function (binding, index) {
                    if (
                      getNestedProperty(msg, binding.bindSource) !== undefined
                    ) {
                      counter++
                    }
                  })
                  node.attributeBasedBindings.forEach(function (
                    binding,
                    index
                  ) {
                ...
                if (
                  ...
                  if (
                    topicParts[0] !== 'update_text' ||
                    topicParts[0] !== 'update_innerHTML'
                  ) {
                    node.error(
                      "Only msg.topic 'update_text' or 'update_innerHTML' is supported (msg._msgid = '" +
                        msg._msgid +
                        "')"
                    )
                    msg.payload = null
                  ...
                  if (Array.isArray(msg.payload)) {
                    for (var i = 0; i < msg.payload.length; i++) {
                      ...
                      if (typeof part === 'object' && !part.command) {
                        node.error(
                          "The msg.payload array should contain objects which all have a 'command' property (msg._msgid = '" +
                            msg._msgid +
                            "')"
                        )
                      ...
                      if (
                        !node.availableCommands.includes(
                          part.command.toLowerCase()
                        )
                      ) {
                        node.error(
                          "The msg.payload array contains an object that has an unsupported command property '" +
                            part.command +
                            "' (msg._msgid = '" +
                            msg._msgid +
                            "')"
                    ...
                    if (typeof msg.payload === 'object') {
                      if (!msg.payload.command) {
                        node.error(
                          "The msg.payload should contain an object which has a 'command' property (msg._msgid = '" +
                            msg._msgid +
                            "')"
                        )
                        msg.payload = null
                      ...
                      // Make sure the commands are not case sensitive anymore
                        ...
                        !node.availableCommands.includes(
                          msg.payload.command.toLowerCase()
                        )
                      ) {
                        node.error(
                          "The msg.payload contains an object that has an unsupported command property '" +
                            msg.payload.command +
                            "' (msg._msgid = '" +
                            msg._msgid +
                            "')"
          ...
          initController: function ($scope, events) {
            ...
            function handleEvent(evt, proceedWithoutTimer) {
              ...
              if (!svgElement) {
                logError(
                  'No SVG element has been found for this ' +
                    evt.type +
                    ' event'
                )
                return
              ...
              if (!userData) {
                logError(
                  'No user data available for this ' + evt.type + ' event'
                )
                return
              }
              ...
              if (evt.type === 'change') {
                ...
                if (
                  msg.event.pageX !== undefined &&
                  msg.event.pageY !== undefined
                ) {
                  ...
                  try {
                    ...
                    logError(
                      'No bounding client rect has been found for this ' +
                        evt.type +
                        ' event'
                    )
                    return
            ...
            function handleJsEvent(evt, proceedWithoutTimer) {
              ...
              var userData = svgElement.getAttribute(
                'data-js_event_' + evt.type
              )

              if (!userData) {
                logError(
                  'No user data available for this ' +
                    evt.type +
                    ' javascript event'
                )
                return
              ...
              try {
                ...
                if ($scope.config.enableJsDebugging) {
                  debugger
                }
            ...
            function applyEventHandlers(rootElement) {
              if ($scope.config.clickableShapes) {
                ...
                $scope.config.clickableShapes.forEach(function (
                  clickableShape
                ) {
                  ...
                  var elements = rootElement.querySelectorAll(
                    clickableShape.targetId
                  ) // The "targetId" now contains the CSS selector!
                  ...
                  elements.forEach(function (element) {
                    ...
                    if (
                      /*!config.showMouseLines && */ action ==
                      'click' /*&& !element.style.cursor*/
                    ) {
                      element.style.cursor = 'pointer'
                    }
            ...
            function initializeSvg(scope) {
              ...
              scope.config.smilAnimations.forEach(function (smilAnimation) {
                ...
                var element = scope.rootDiv.querySelector(
                  '#' + smilAnimation.targetId
                )
                ...
                if (element) {
                  ...
                  if (smilAnimation.attributeName === 'transform') {
                    animationElement = document.createElementNS(
                      'http://www.w3.org/2000/svg',
                      'animateTransform'
                    )
                    animationElement.setAttribute(
                      'type',
                      smilAnimation.transformType
                    )
                  } else {
                    animationElement = document.createElementNS(
                      'http://www.w3.org/2000/svg',
                  ...
                  animationElement.setAttribute(
                    'class',
                    smilAnimation.classValue
                  )
                  animationElement.setAttribute(
                    'attributeName',
                    smilAnimation.attributeName
                  )
                  if (smilAnimation.fromValue != '')
                    animationElement.setAttribute(
                      'from',
                      smilAnimation.fromValue
                    ) //permit transition from current value if not specified
                  animationElement.setAttribute('to', smilAnimation.toValue)
                  animationElement.setAttribute(
                    'dur',
                    smilAnimation.duration + (smilAnimation.durationUnit || 's')
                  ) // Seconds e.g. "2s"
                  ...
                  switch (smilAnimation.trigger) {
                    case 'time':
                      ...
                      animationElement.setAttribute(
                        'begin',
                        smilAnimation.delay + (smilAnimation.delayUnit || 's')
                      )
                      break
                    case 'cust':
                      animationElement.setAttribute(
                        'begin',
                        smilAnimation.custom
                      )
                      break
                    default:
                  ...
                  logError(
                    "No animatable element found for selector '" +
                      smilAnimation.targetId +
                      "'"
                  )
                }
              ...
              if (scope.config.showCoordinates && scope.svg) {
                scope.tooltip = document.getElementById(
                  'tooltip_' + scope.config.nodeIdWithoutDot
                )
                scope.svg.addEventListener(
                  'mousemove',
                  function (evt) {
                    ...
                    if (
                      tooltipX >
                      mdCardElement.clientWidth - scope.tooltip.clientWidth - 20
                    ) {
                    ...
                    if (
                      tooltipY >
                      mdCardElement.clientHeight -
                        scope.tooltip.clientHeight -
                        20
                    ) {
            ...
            $scope.init = function (config) {
              $scope.config = config
              $scope.faMapping = {}
              $scope.rootDiv = document.getElementById(
                'svggraphics_' + config.nodeIdWithoutDot
              )
              ...
              if (panning !== 'disabled' || zooming !== 'disabled') {
                ...
                if (config.mouseWheelZoomEnabled) {
                  $scope.svg.parentElement.addEventListener(
                    'wheel',
                    $scope.panZoomModule.zoomWithWheel
                  )
                }
                ...
                if (config.doubleClickZoomEnabled) {
                  ...
                  $scope.mc.on('doubletap', function (evt) {
                    ...
                    var dblClickZoomPercentage = parseInt(
                      $scope.config.dblClickZoomPercentage || 100
                    )
                    ...
                    $scope.panZoomModule.zoomToPoint(dblClickZoomFactor, {
                      clientX: evt.center.x,
                      clientY: evt.center.y,
                    })
                  })
                }
              }
              ...
              function (msg) {
                ...
                function processCommand(_msgid, payload, topic) {
                  ...
                  try {
                    $scope.config.javascriptHandlers.forEach(function (
                      ...
                      if (javascriptHandler.action === 'msg') {
                        try {
                          ...
                          logError(
                            "Error in javascript input msg handler (msg._msgid = '" +
                              _msgid +
                              "'): " +
                              err
                          )
                    ...
                    if (topic) {
                      if (topic == 'custom_msg') {
                        ...
                        bindings.forEach(function (binding) {
                          ...
                          var elements = $scope.rootDiv.querySelectorAll(
                            binding.selector
                          )
                        ...
                        //
                        textElements =
                          $scope.rootDiv.querySelectorAll('[data-bind-text]')
                        ...
                        //
                        attrElements = $scope.rootDiv.querySelectorAll(
                        ...
                        if (!attrElements || !attrElements.length) {
                          ...
                          attrElements.forEach(function (element) {
                            var attributesCSV = element.getAttribute(
                              'data-bind-attributes'
                            )
                            var attrBindToCSV =
                              element.getAttribute('data-bind-values')
                            if (attributesCSV && attrBindToCSV) {
                              ...
                              if (attrNames.length != attrBindTos.length) {
                                logError(
                                  "data-bind-attributes count is different to data-bind-values count (msg._msgid = '" +
                                    _msgid +
                                    "')"
                                )
                                return
                        ...
                        if (topicParts.length > 1) {
                          if (
                            ...
                            if (!elements || !elements.length) {
                              logError(
                                'Invalid selector. No SVG elements found for selector ' +
                                  selector +
                                  "(msg._msgid = '" +
                                  _msgid +
                                  "')"
                    ...
                    switch (op.toLowerCase()) {
                      case 'replace_svg':
                        if (!payload.svg || typeof payload.svg !== 'string') {
                          logError(
                            "Invalid payload. The payload should be an SVG string (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        try {
                          ...
                          logError(
                            "Invalid payload.svg.  No valid SVG string (msg._msgid = '" +
                              _msgid +
                              "'): " +
                              err
                          )
                        ...
                        if (newSvg.tagName !== 'svg') {
                          logError(
                            "Invalid payload. The tag of the first element should be 'svg' (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        while ($scope.svg.attributes.length > 0) {
                          $scope.svg.removeAttribute(
                            $scope.svg.attributes[0].name
                          )
                        }
                      ...
                      case 'add_element': // Add elements, or replace them if they already exist
                        if (!payload.elementType) {
                          logError(
                            "Invalid payload. A property named .elementType is not specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        if (payload.parentSelector || payload.parentElementId) {
                          ...
                          parentElements =
                            $scope.rootDiv.querySelectorAll(selector)
                        }
                        ...
                        if (parentElements.length > 1 && payload.elementId) {
                          logError(
                            "When multiple parent SVG elements are specified, it is not allowed to specify an .elementId (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        parentElements.forEach(function (parentElement) {
                          ...
                          if (payload.elementStyleAttributes) {
                            ...
                            for (const [key, value] of Object.entries(
                              payload.elementStyleAttributes
                            )) {
                              style += key
                              style += ':'
                              style += value
                      ...
                      case 'remove_element':
                        if (!payload.elementId && !payload.selector) {
                          logError(
                            "Invalid payload. A property named .elementId or .selector is not specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                      ...
                      case 'update_innerhtml': //added to make adding inner HTML more readable/logical
                        ...
                        var innerContent =
                          payload.text || payload.html || payload.textContent
                        elements.forEach(function (element) {
                      ...
                      case 'set_style': //same as update_style
                        ...
                        if (payload.style && $scope.isObject(payload.style)) {
                          ...
                          logError(
                            "Cannot update style! style object not valid or attributeName/attributeValue strings not provided (selector '" +
                              selector +
                              "' and msg._msgid = '" +
                              _msgid +
                              "')"
                      ...
                      case 'update_value':
                        ...
                        elements.forEach(function (element) {
                          if (element.value === undefined) {
                            logError(
                              "An SVG element selected by '" +
                                selector +
                                "' has no 'value' property (msg._msgid = '" +
                                _msgid +
                                "')"
                            )
                            return
                      ...
                      case 'replace_all_attribute':
                        ...
                        if (!payload.regex) {
                          logError(
                            "Invalid payload. A regular expression should be specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        if (!payload.replaceValue) {
                          logError(
                            "Invalid payload. A replace value should be specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        elements.forEach(function (element) {
                          ...
                          var regex =
                            op == 'replace_attribute'
                              ? new RegExp(payload.regex)
                              : new RegExp(payload.regex, 'g')

                          if (!regex.test(attributeValue)) {
                            logError(
                              'The value of attribute ' +
                                payload.attributeName +
                                " does not match the regex (msg._msgid = '" +
                                _msgid +
                                "')"
                            )
                            return
                          ...
                          var replacedAttribute = attributeValue.replace(
                            regex,
                      ...
                      case 'trigger_animation':
                        ...
                        if (!payload.action) {
                          logError(
                            "When triggering an animation, there should be a .action field (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                      ...
                      case 'remove_event': // remove the specified event(s) from the specified element(s)
                        ...
                        if (!payload.event) {
                          logError(
                            "No msg.payload.event has been specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        if (!$scope.events.includes(payload.event)) {
                          logError(
                            "The msg.payload.event contains an unsupported event name (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        elements.forEach(function (element) {
                          ...
                          var userData = element.getAttribute(
                            'data-event_' + payload.event
                          )
                          ...
                          if (op === 'add_event') {
                            if (userData) {
                              logError(
                                'The event ' +
                                  payload.event +
                                  " already has been registered (msg._msgid = '" +
                                  _msgid +
                                  "')"
                              )
                            } else {
                            ...
                            if (!userData) {
                              logError(
                                'The event ' +
                                  payload.event +
                                  " was not registered yet (msg._msgid = '" +
                                  _msgid +
                                  "')"
                              )
                            } else {
                      ...
                      case 'remove_js_event': // remove the specified Javascript event(s) from the specified element(s)
                        ...
                        if (!$scope.events.includes(payload.event)) {
                          logError(
                            "The msg.payload.event contains an unsupported javascript event name (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                      ...
                      case 'zoom_in':
                        if (!$scope.panZoomModule) {
                          logError(
                            "Cannot zoom via input message, when zooming is not enabled in the settings (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                      ...
                      case 'zoom_by_percentage':
                        ...
                        if (!payload.percentage) {
                          logError(
                            "No msg.payload.percentage has been specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        if (payload.x && payload.y) {
                          ...
                          var clientX =
                            $scope.svg.getBoundingClientRect().x + payload.x
                          var clientY =
                            $scope.svg.getBoundingClientRect().y + payload.y

                          // When a point has been specified, zoom by the specified percentage at the specified point
                          $scope.panZoomModule.zoomToPoint(factor, {
                            clientX: clientX,
                            clientY: clientY,
                          })
                        } else {
                      ...
                      case 'pan_to_point':
                        if (!$scope.panZoomModule) {
                          logError(
                            "Cannot pan via input message, when panning is not enabled in the settings (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                        ...
                        if (!payload.x || !payload.y) {
                          logError(
                            "No point coordinates (msg.payload.x msg.payload.y) have been specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                      ...
                      case 'pan_to_direction':
                        ...
                        if (!payload.x || !payload.y) {
                          logError(
                            "No direction coordinates (msg.payload.x msg.payload.y) have been specified (msg._msgid = '" +
                              _msgid +
                              "')"
                          )
                          return
                    ...
                    logError(
                      "Unexpected error when processing input message (msg._msgid = '" +
                        _msgid +
                        "'): " +
                        error
                    )
                ...
                if (!payload || payload === '') {
                  logError(
                    'Missing msg.payload (msg._msgid = ' + msg._msgid + ')'
                  )
                  return
                }
                ...
                if (topic == 'custom_msg') {
                  ...
                  payload.forEach(function (val, idx) {
                    if (typeof val != 'object' || !val.command) {
                      logError(
                        "The msg.payload should contain an object (or an array of objects) which have a 'command' property (msg._msgid = " +
                          msg._msgid +
                          ')'
                      )
                    } else {
  ...
  RED.httpAdmin.get(
    '/ui_svg_graphics/image/:nodeid/:relativefilepath',
    RED.auth.needsPermission('ui_svg_graphics.read'),
    function (req, res) {
      ...
      if (!svgNode.directory || svgNode.directory === '') {
        res.writeHead(404)
        return res.end(
          'No local image directory has been specified for node with id = ' +
            nodeId
        )
      }
